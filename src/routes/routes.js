...
import EditExam from "../components/partials/forms/editExamContent.form";
...
import ManageStudent from "../components/pages/teachers/managestudents";
	...
	routes: [
		{
			...
			beforeEnter: (to, from, next) => {
				if (store.state.authModule.user_key == null) next("/login");
				else next();
			}
		},
		{
			...
			component: LoginForm,
			beforeEnter: (to, from, next) => {
				if (store.state.authModule.user_key != null)
					next({ name: "Homepage" });
				else next();
			}
		},
