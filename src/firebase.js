...
export const loadStudentsByExamCodeOfTeacher = async arrQuery => {
	// teacher_email
	// fetch all exmcode registered by logged-in teacher
	...
	// fetch students by exam code of logged-in teacher
	if (arrQuery.sortBy == "" || arrQuery.sortBy == "resetSort") {
		...
		fetchStudentsByExamStatus(arrQuery.sortBy).then(result => {
			if (result == 0) {
				return;
			} else {
				result.forEach(singleItem => {
...
export const searchStudent = async searchKeyword => {
	console.log(searchKeyword); // use algolia
	// const listStudentsByExamCode = await usersdb
...
export const fetchStudentsEnrolledFb = async examCode => {
	...
	await requestexamdb
		...
		.get()
		.then(querySnapshot => {
			getStudentIds.value = querySnapshot.docs.map(doc => ({
				requestID: doc.id,
	...
	getStudentIds.value.forEach(result => {
		getUser(result.student_id).then(userInfo => {
			listStudentsEnrolledByExamCode.value.push({
				...
				status: result.status_of_request
			});
		});
	});

	return listStudentsEnrolledByExamCode.value;
};
...
export const fetchStudentsByExamStatus = async examStatus => {
	const listStudentsByExamStatus = ref([]);
	await requestexamdb
		.where("status_of_request", "==", examStatus)
		.get()
		.then(querySnapshot => {
			listStudentsByExamStatus.value = querySnapshot.docs.map(doc => ({
				requestID: doc.id,
				dateRequested: doc.data()["dateRequested"],
				timeRequested: doc.data()["timeRequested"],
				examCode: doc.data()["exam_code"],
	...
	if (listStudentsByExamStatus.value.length == 0) {
		return 0;
	} else {
		return listStudentsByExamStatus.value;
	}
};
